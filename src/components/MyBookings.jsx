...
export default function MyBookings({ setTab, refresh, setRefresh }) {
  ...
  const card = (item, canCancel) => <article className="glass-card booking-card" key={item.id}><div><b>{item.beauty_services?.name}</b><strong>{money(item.beauty_services?.price)}</strong></div><p><CalendarIcon size={14}/>{formatDateTime(item.starts_at)}</p>{canCancel && <button className="danger-button" onClick={() => cancel(item.id)}>Отменить запись</button>} {!canCancel && <span className={`status ${item.status}`}>{item.status === 'completed' ? 'Выполнено' : item.status === 'cancelled' ? 'Отменено' : 'Прошедшая'}</span>}</article>;
